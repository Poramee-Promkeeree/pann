...
export class UserResultRepository implements IRepository<UserResult> {
    ...
    async toggleIsPinned(id: string|number, pin: boolean): Promise<UserResult | null> {
        const endpoint = pin ? '/pin/1' : '/pin/0'
        const resp = await ax.get<UserResult>(`${this.urlPrefix}/userResult/${id}${endpoint}`)
        return resp.data
    }
